...
const formatCustomerProduct = (product, sizes = []) => {
  ...
  return {
    ...
    material: (product.material && /cotton/i.test(product.material)) ? 'Cotton' : (product.material || 'Cotton'),
    weaveType: product.weaveType || '2/20s Ring',
    gsmRange: `${product.weaveType || '2/20s'} • Cotton`,
    fastness: 'Grade 4+ Cl',
